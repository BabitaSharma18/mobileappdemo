...
export default function DownloadOrReturnProductImage(context) {
	...
	if (fileExists(fileName)) {
		// media is local and can be opened
		 ...
		 return context.executeAction("/samplemdk/Actions/mobileappazeem/Products/OpenProductImage.action");
	} else {
		pageProxy.setActionBinding(bindingObject);
		return context.executeAction("/samplemdk/Actions/mobileappazeem/Products/DownloadProductImage.action").then((result) => {
			// success case
