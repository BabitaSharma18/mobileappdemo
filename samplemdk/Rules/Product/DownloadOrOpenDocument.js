...
export default async function DownloadOrOpenDocument(sectionedTableProxy) {
	...
	const serviceName = "/samplemdk/Services/mobileappazeem.service";
	...
	if (mediaIsLocal && fileExists) {
		...
		return pageProxy.executeAction("/samplemdk/Actions/mobileappazeem/Products/OpenProductImage.action");
		...
		return pageProxy.executeAction("/samplemdk/Actions/mobileappazeem/Products/DownloadImage.action");
	} 
	else {
		let pressedItem = pageProxy.getPressedItem();
		let section = sectionedTableProxy.getSections()[0];
		section.setIndicatorState("inProgress", pressedItem);
		await sectionedTableProxy.executeAction("/samplemdk/Actions/mobileappazeem/Products/DownloadDocumentStreams.action").catch(error => {
